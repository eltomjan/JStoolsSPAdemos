...
3-8 50-55 ?       50-55   9-14 199-230 50-55 98-103 15-20 116-121 57-68 68-73  98-103 98-103 21-26
	87-92 135-166 193-8                57-68 45-50        122-127       99-104 75-80  81-86
		 '167-198 '218-29                                 129-134
		 '39-44
P     Q      R     S       T     U     V       W       X      Y     Z
87-92 56-67  32-37 135-166 193-8 27-32 98-103  98-103  56-67  15-20 98-103
	  98-103 93-98                     104-109 104-109 135-166      134-9
	  104-109                          111-6   111-6                136-41
	  111-116                                                       139-44 */
...
OUTLAST:
	...
	for (unsigned i = 0; i < (unsigned)DEwait; i += 9) {
		if (avgTab[avg] > 128) avg--; else if (avgTab[avg] < 128) avg++;
		fputc(avgTab[avg], f);
	};
	C--;
	if (A = C & 0xF)
...
int main()
{
	...
	unsigned char *HL = (unsigned char*)//"JSEM SINKLE'R ZET IKS SPEKTRUM POC'I'TAC' VY SOKY'CH KVALIT ZADEJ SVU'J TEKST";
		"KARIE'RA VE VITESCO TECHNOLOGIES TVAR ZI'TRA S NA'MI. NYNI'. NABI'ZI'ME S'IROKOU S'KA'LU PRACOVNI'CH MI'ST PRO VA'S'NIVE' PRU'KOPNI'KY V CELE' GLOBA'LNI' ORGANIZACI HM";
	SAY_IT(HL, f);
...
#define WAVFILE_SAMPLES_PER_SECOND BITRATE
#endif
