...
class PDFPageView extends BasePDFPageView {
  ...
  reorder(_src) {
    ...
    let els = [];
    const elDest = [];
    const scale = parseFloat(this._container.style.getPropertyValue("--scale-factor"));
    ...
    function XL() {
      ...
      for (let i = 1; i < elList.length; i++) {
        if (elList[i][0] <= xOld[1] || elList[i][3] > xOld[4]) {
          xl += '\n';
      ...
      let ta;
      if (document.getElementsByTagName('TEXTAREA').length === 2) ta = document.getElementsByTagName('TEXTAREA')[1];else {
    ...
    if (!this.textLayer && this.#textLayerMode !== TextLayerMode.DISABLE && !pdfPage.isPureXfa) {
      ...
      this.textLayer = new TextLayerBuilder({
        ...
        onAppend: textLayerDiv => {
          this.l10n.pause();
          this.reorder(textLayerDiv);
          this.#addLayer(textLayerDiv, "textLayer");
          if (document.activeElement.constructor !== HTMLInputElement) this.#layers[1].firstElementChild.focus();
          this.l10n.resume();
        }
      });
    }
